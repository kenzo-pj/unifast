...
`@unifast/plugin-shiki` provides syntax highlighting powered by [Shiki](https://shiki.style/), supporting 200+ languages and a wide variety of themes. Unlike the built-in highlight plugin, Shiki runs as a HAST transform in JavaScript, replacing `<pre><code>` blocks with richly styled output.

The plugin automatically disables the built-in Rust highlighter to avoid double-processing.

## Installation

<PackageInstall package="@unifast/plugin-shiki" />
