import type { HastNode, HastRoot, HastElement } from "@unifast/core";
...
function convertNode(
  ...
  switch (node.type) {
    ...
    case "text":
      return node.value;
    case "raw":
      // raw nodes contain pre-processed HTML from the compiler pipeline;
      // users should enable the sanitize plugin if input is untrusted
      return createElement("div", {
        key,
        dangerouslySetInnerHTML: { __html: node.value }, // SECURITY: relies on unifast sanitize pass for untrusted input
      });
    case "comment":
    case "doctype":
      return null;
