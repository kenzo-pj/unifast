...
export interface UnifastPluginOptions {
  /** Options passed directly to `@unifast/node` `compile()`. */
  md?: CompileOptions;
  /** Options passed to `compile()` for `.mdx` files. Falls back to `md` if not set. */
  mdx?: CompileOptions;
}

type CompileOptions = Record<string, unknown>;

type CompileFn = (input: string, opts?: CompileOptions) => {
  output: string;
  frontmatter: Record<string, unknown>;
  toc: Array<{ depth: number; text: string; slug: string }>;
};

// ── Resolve compile function ─────────────────────────────────────────────
  ...
  return {
    ...
    transform(_code, id) {
      ...
      if (isMdx) {
        ...
        return compile
          ? transformMdx(source, compile, compileOpts)
          : null;
      }
...
/**
 * Post-process MDX JS output to replace plain code blocks with
 * highlighted versions.
 *
 * The MdxJs emitter re-parses from MDAST and bypasses HAST transforms
 * (highlight, line_number), producing unhighlighted code blocks like:
 *   _jsx("code", { children: "...", className: "language-ts" })
 *
 * This function finds those patterns, compiles each code block as
 * standalone Markdown with highlighting, and replaces them with
 * dangerouslySetInnerHTML to render the pre-highlighted HTML.
 *
 * Security: The HTML is produced by the Rust sanitize pass from
 * trusted local .mdx content files, not user-supplied input.
 */
  ...
  return jsOutput.replace(
    ...
    (match, escapedCode, lang) => {
      try {
        ...
        // Safe: HTML is generated by Rust compiler from trusted local content
